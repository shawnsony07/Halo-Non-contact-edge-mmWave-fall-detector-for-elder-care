...
static const uint32_t TLV_TYPE_COMPRESSED_POINTS = 1020; // Added for compressed cloud
...
void logTlvTypeOnce(uint32_t tlvType, uint32_t tlvLength) {
  for (size_t i = 0; i < seenTlvTypes.size(); i++) {
    if (seenTlvTypes[i] == tlvType) return; 
  }
...
void parseBuffer() {
  ...
  while (true) {
    ...
    for (size_t i = pos; i + 8 <= buffer.size(); i++) {
      if (memcmp(&buffer[i], MAGIC_WORD, 8) == 0) { syncIdx = (long)i; break; }
    }
    if (syncIdx == -1) {
      if (buffer.size() > 8) buffer.erase(buffer.begin(), buffer.end() - 8);
      break;
    }
    if (syncIdx > 0) buffer.erase(buffer.begin(), buffer.begin() + syncIdx);
    ...
    for (uint32_t i = 0; i < numTlvs; i++) {
      ...
      if (tlvType == TLV_TYPE_COMPRESSED_POINTS && tlvLength > 20) {
        ...
        if (numPoints > MAX_POINTS_PER_TLV) numPoints = MAX_POINTS_PER_TLV; 
      ...
      // ==========================================
      if (tlvType == TLV_TYPE_TARGET_LIST && tlvLength > 0 && tlvLength % TARGET_STRUCT_SIZE == 0) {
      ...
      if (tlvType == TLV_TYPE_VITALS && tlvLength >= 16) {
        ...
        bool heartValid = (heartRate == 0.0f) || (heartRate >= 30.0f && heartRate <= 220.0f);
        bool breathValid = (breathRate == 0.0f) || (breathRate >= 3.0f && breathRate <= 60.0f);
        bool devValid = fabsf(breathDeviation) <= 100.0f;
        if (!heartValid || !breathValid || !devValid) continue;
